document.addEventListener('DOMContentLoaded', () => {
    ...
    const Game = {
        init() {
            ...
            this.stageData = {
                obstacles: [
                    { x: 0, y: 550, width: 400, height: 50 }, // スタート
                    { x: 650, y: 450, width: 200, height: 40 }, // 飛翔ジャンプ
                    ...
                    { x: 2000, y: 500, width: 2000, height: 100 }, // 弾幕エリア足場
                    { x: 3800, y: 300, width: 150, height: 30}, // 上の砲台足場
                    { x: 4200, y: 450, width: 100, height: 30 }, // アスレチック開始
                    { x: 4400, y: 400, width: 80, height: 30 }, { x: 4600, y: 450, width: 80, height: 30 },
                    { x: 4850, y: 380, width: 60, height: 30 }, { x: 5050, y: 460, width: 60, height: 30 },
                    { x: 5250, y: 350, width: 150, height: 30 }, // アスレチック中間
                    { x: 5550, y: 420, width: 60, height: 30 }, { x: 5750, y: 350, width: 60, height: 30 },
                    { x: 5950, y: 450, width: 80, height: 30 }, { x: 6150, y: 400, width: 80, height: 30 },
                    { x: 6600, y: 520, width: 2000, height: 80 }, // 敵ラッシュ地帯
                    { x: 8800, y: 400, width: 150, height: 40 }, // 終盤開始
                    { x: 9100, y: 280, width: 150, height: 40 },
                    { x: 9400, y: 450, width: 150, height: 40 },
                    { x: 9700, y: 350, width: 40, height: 150 }, // 壁
                    { x: 9900, y: 250, width: 200, height: 40 },
                    { x: 10300, y: 500, width: 300, height: 100},
                    { x: 10350, y: 350, width: 200, height: 40},
                    { x: 10800, y: 450, width: 1000, height: 150}, // ゴール前足場
                ],
                fixedEnemies: [
                    { type: 'turret', x: 2200, y: 440 }, { type: 'turret', x: 2800, y: 440 },
                    { type: 'turret', x: 3500, y: 440 }, { type: 'turret', x: 3825, y: 240 },
                    { type: 'turret', x: 9125, y: 220 }, { type: 'turret', x: 9425, y: 390 },
                    { type: 'turret', x: 10400, y: 290 },{ type: 'turret', x: 10500, y: 440 }
                ],
                groundEnemies: [
                    { type: 'onibi', x: 6800, y: 520 }, { type: 'onibi', x: 7100, y: 520 },
                    { type: 'onibi', x: 7400, y: 520 }, { type: 'onibi', x: 7700, y: 520 },
                    { type: 'onibi', x: 8000, y: 520 }, { type: 'onibi', x: 8300, y: 520 }
                ],
                damageZones: [
                    { x: 4200, y: 580, width: 2350, height: 20 }
                ],
                healItems: [
                    { x: 4100, y: 450 }, { x: 8700, y: 450 }, { x: 11600, y: 350 }
                ],
                goalX: 11800,
            };
        ...
        // (initAudio以降のメソッドはstage1.jsと同じなので省略)
        // ...
        // ... stage1.jsのinitAudioからresetRideCooldownVisualまでの全メソッドをここにコピー ...
        initAudio() {
            this.audioCtx = null;
            this.audio = {
        ...
        playBGM() {
            if (!this.audio.bgm) return; this.resumeAudioContext(); const playPromise = this.audio.bgm.play();
        ...
        setupEventListeners() {
            document.addEventListener('keydown', e => this.keys[e.code] = true); document.addEventListener('keyup', e => this.keys[e.code] = false);
            ...
            window.addEventListener('gamepadconnected', e => this.handleGamepadConnected(e)); window.addEventListener('gamepaddisconnected', e => this.handleGamepadDisconnected(e));
        },
        startNewGame() {
            ...
            this.player.reset(); this.gameObjects = [this.player]; this.createStage(); this.gameState = 'playing'; this.gauge = 0; this.score = 0; this.updateScoreDisplay(); this.cameraX = 0; this.enemySpawnTimer = 3000; this.updateHpBar(); this.updateGaugeBar(); this.resetRideCooldownVisual(); this.gameOverScreen.classList.add('hidden'); this.gameClearScreen.classList.add('hidden'); this.container.classList.remove('screen-shake'); this.stopBGM(); Object.values(this.audio).forEach(audioEl => audioEl && audioEl.pause()); this.playBGM();
        },
        createStage() { 
    ...
    // GameObject以下のクラス定義は、stage1.jsから変更ありません。
    // (長いので省略...stage1.jsのGameObject以降のクラス定義をここに貼り付けてください)
