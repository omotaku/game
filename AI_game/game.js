document.addEventListener('DOMContentLoaded', () => {
    ...
    const Game = {
        init() {
            ...
            this.stageData = {
                obstacles: [ { x: 50, y: 500, width: 300, height: 100 }, { x: 450, y: 450, width: 150, height: 40 }, { x: 700, y: 400, width: 120, height: 40 }, { x: 950, y: 450, width: 200, height: 40 }, { x: 1250, y: 380, width: 80, height: 40 }, { x: 1400, y: 320, width: 80, height: 40 }, { x: 1550, y: 260, width: 80, height: 40 }, { x: 1750, y: 350, width: 300, height: 40 }, { x: 2150, y: 300, width: 40, height: 40 }, { x: 2300, y: 250, width: 40, height: 40 }, { x: 2450, y: 200, width: 40, height: 40 }, { x: 2600, y: 400, width: 250, height: 40 }, { x: 2950, y: 400, width: 250, height: 40 }, { x: 3300, y: 350, width: 150, height: 150 }, { x: 3600, y: 250, width: 100, height: 40 }, { x: 3800, y: 200, width: 100, height: 40 }, { x: 4000, y: 150, width: 100, height: 40 }, { x: 4200, y: 400, width: 400, height: 200 }, { x: 4700, y: 450, width: 150, height: 40 } ],
                fixedEnemies: [ { type: 'turret', x: 1000, y: 390 }, { type: 'turret', x: 1800, y: 290 }, { type: 'turret', x: 2800, y: 340 }, { type: 'turret', x: 3100, y: 340 }, { type: 'turret', x: 4400, y: 340 }, { type: 'turret', x: 5800, y: 290 }, { type: 'turret', x: 6700, y: 440 } ],
                movingPlatforms: [ { startX: 4900, startY: 450, endX: 5200, endY: 450, width: 120, height: 40, speed: 2.25 }, { startX: 5400, startY: 400, endX: 5400, endY: 200, width: 80, height: 40, speed: 2.5 }, ],
                disappearingPlatforms: [ { x: 5600, y: 350, width: 80, height: 40 }, { x: 5750, y: 400, width: 80, height: 40 }, { x: 5900, y: 450, width: 80, height: 40 }, { x: 6100, y: 450, width: 80, height: 40 }, { x: 6300, y: 450, width: 80, height: 40 }, { x: 6500, y: 500, width: 300, height: 40 } ],
        ...
        startNewGame() {
            document.querySelectorAll('.enemy, .ofuda, .small-ofuda, .special-ofuda, .bomb, .explosion, .obstacle, .goal, .heal-item, .enemy-bullet, .fireball, .moving-platform, .disappearing-platform, .damage-zone').forEach(el => el.remove());
            this.player.reset(); this.gameObjects = [this.player]; this.createStage(); this.gameState = 'playing'; this.gauge = 0;
            this.score = 0;
            this.updateScoreDisplay();
            this.cameraX = 0; this.enemySpawnTimer = 3000; this.updateHpBar(); this.updateGaugeBar(); this.updateOfudaRideIndicator(true); this.gameOverScreen.classList.add('hidden'); this.gameClearScreen.classList.add('hidden');
        },
        createStage() { this.world.style.width = `${this.WORLD_WIDTH}px`; this.stageData.obstacles.forEach(d => this.add(new Obstacle(d.x, d.y, d.width, d.height))); this.stageData.fixedEnemies.forEach(d => { if (d.type === 'turret') this.add(new TurretEnemy(d.x, d.y)); }); this.stageData.movingPlatforms.forEach(d => this.add(new MovingPlatform(d.startX, d.startY, d.endX, d.endY, d.width, d.height, d.speed))); this.stageData.disappearingPlatforms.forEach(d => this.add(new DisappearingPlatform(d.x, d.y, d.width, d.height))); this.stageData.groundEnemies.forEach(d => { if(d.type === 'onibi') this.add(new OnibiEnemy(d.x, d.y)); }); this.stageData.damageZones.forEach(d => this.add(new DamageZone(d.x, d.y, d.width, d.height))); this.add(new Goal(this.WORLD_WIDTH - 200, 200, 100, 200)); },
        gameLoop(currentTime) { const deltaTime = Math.min(currentTime - this.lastTime, 100); this.lastTime = currentTime; if (this.gameState === 'playing') { this.update(deltaTime); this.draw(); } requestAnimationFrame((time) => this.gameLoop(time)); },
        update(deltaTime) {
            this.updateInputState();
            this.enemySpawnTimer -= deltaTime; if (this.enemySpawnTimer <= 0) { this.spawnEnemy(); this.enemySpawnTimer = 2500 + Math.random() * 2000; } this.gameObjects.forEach(obj => obj.update(deltaTime)); this.updateCamera(); this.gameObjects = this.gameObjects.filter(obj => !obj.isDestroyed);
        },
        updateInputState() {
            ...
            if (this.gamepadIndex !== null) {
                ...
                if (gamepad) {
                    const deadZone = 0.3;
                    this.inputState.left = this.inputState.left || gamepad.axes[0] < -deadZone || gamepad.buttons[14].pressed;
                    this.inputState.right = this.inputState.right || gamepad.axes[0] > deadZone || gamepad.buttons[15].pressed;
                    this.inputState.jump = this.inputState.jump || gamepad.buttons[0].pressed;
                    this.inputState.fly = this.inputState.fly || gamepad.buttons[1].pressed;
                    this.inputState.shoot = this.inputState.shoot || gamepad.buttons[2].pressed;
                    this.inputState.guard = this.inputState.guard || gamepad.buttons[4].pressed; // L1/LB for guard
        ...
        handleGameOver() { this.gameState = "gameOver"; this.gameOverScreen.classList.remove("hidden"); this.player.destroy(); },
        handleGameClear() { if (this.gameState === 'playing') { this.gameState = "cleared"; this.gameClearScreen.classList.remove("hidden"); this.player.vx = 0; } },
        updateOfudaRideIndicator(isReady) { if (isReady) { this.ofudaRideIndicator.classList.remove('indicator-cooldown'); this.ofudaRideIndicator.classList.add('indicator-ready'); } else { this.ofudaRideIndicator.classList.remove('indicator-ready'); this.ofudaRideIndicator.classList.add('indicator-cooldown'); } }
    };
    ...
    class Player extends GameObject {
        ...
        // ▼▼▼ 変更: ガード開始条件にゲージ残量チェックを追加 ▼▼▼
        handleGuard() {
            const canGuard = Game.inputState.guard && !this.isRidingOfuda && !this.isChargingAttack && !this.isGuardBroken && Game.gauge > 0;
            if (canGuard) {
                if (!this.isGuarding) {
                    this.isGuarding = true;
                    this.guardEffect.style.display = 'block';
                }
            } else {
                if (this.isGuarding) {
                    this.isGuarding = false;
        ...
        // ▼▼▼ 変更: ガードブレイク時の無敵化を削除 ▼▼▼
        triggerGuardBreak() {
            if (this.isGuardBroken) return;
            this.isGuardBroken = true;
            this.isGuarding = false;
            this.guardEffect.style.display = 'block';
            this.guardEffect.classList.add('guard-break');
            setTimeout(() => {
                this.isGuardBroken = false;
                this.guardEffect.style.display = 'none';
            ...
            [...enemies, ...projectiles].forEach(hazard => {
                if (Game.checkRectCollision(this, hazard)) {
                    if (this.isGuarding && !this.isInvincible) {
                        const originalDamage = hazard.damage || 15;
                        const damageReductionRate = Game.gauge / 100;
                        const actualDamage = Math.round(originalDamage * (1 - damageReductionRate));

                        if (actualDamage > 0) {
                            this.takeDamage(actualDamage, 0, { noKnockback: true, isGuarded: true });
                        }
                        
                        Game.gauge = Math.max(0, Game.gauge - originalDamage);
                        Game.updateGaugeBar();
                        ...
                        if (hazard.type === 'bomb' || hazard.type === 'enemy-bullet') {
                            if (hazard.type === 'bomb') Game.add(new Explosion(hazard.x, hazard.y));
                            hazard.destroy();
                        }
        ...
        takeDamage(amount, knockbackDirection, options = {}) {
            ...
            Game.updateHpBar();
            if (this.hp <= 0) { Game.handleGameOver(); return; }
            if (options.isContinuous) return;

            this.isInvincible = true;
            this.element.classList.add('invincible');
            if (!options.noKnockback) {
                this.isKnockback = true; this.vy = -8; this.vx = 4 * knockbackDirection; this.vx_inertia = 0;
                setTimeout(() => { this.isKnockback = false; }, 300);
            }
            
            const invincibleTime = options.isGuarded ? 200 : (options.noKnockback ? 500 : 1500);
            setTimeout(() => { this.isInvincible = false; this.element.classList.remove('invincible'); }, invincibleTime);
        }
        draw(){if(this.isDestroyed)return;this.element.style.left=`${this.x}px`;this.element.style.top=`${this.y}px`;this.element.style.transform=`scaleX(${this.direction==="left"?-1:1})`}
    ...
    class SmallOfuda extends GameObject { constructor(playerX, playerY, direction) { super("small-ofuda"); this.type = "small-ofuda"; this.width = 15; this.height = 30; this.damage = 15; const yOffset = playerY + 28; this.x = playerX + (direction === "right" ? 24 : -5); this.y = yOffset; this.speed = 15 * (direction === "right" ? 1 : -1); } update() { this.x += this.speed; (this.x < Game.cameraX - 50 || this.x > Game.cameraX + Game.WIDTH + 50) && this.destroy(); } }
    class Ofuda extends GameObject { constructor(playerX, playerY, direction) { super("ofuda"); this.type = "ofuda"; this.width = 20; this.height = 40; this.damage = 30; const yOffset = playerY + 28; this.x = playerX + (direction === "right" ? 24 : -12); this.y = yOffset; this.speed = 12 * (direction === "right" ? 1 : -1); } update() { this.x += this.speed; (this.x < Game.cameraX - 50 || this.x > Game.cameraX + Game.WIDTH + 50) && this.destroy(); } }
    class SpecialOfuda extends GameObject { constructor(playerX, playerY, direction) { super("special-ofuda"); this.type = "special-ofuda"; this.width = 40; this.height = 80; this.damage = 100; const yOffset = playerY + 32; this.x = playerX + (direction === "right" ? 24 : -32); this.y = yOffset - this.height / 2; this.speed = 15 * (direction === "right" ? 1 : -1); } update() { this.x += this.speed; (this.x < Game.cameraX - 50 || this.x > Game.cameraX + Game.WIDTH + 50) && this.destroy(); } }
    class Enemy extends GameObject { constructor(className, hp) { super(`enemy ${className}`); this.type = "enemy"; this.hp = hp; this.maxHp = hp; const hpBar = document.createElement("div"); hpBar.className = "enemy-hp-bar"; this.hpBarInner = document.createElement("div"); this.hpBarInner.className = "enemy-hp-bar-inner"; hpBar.appendChild(this.hpBarInner); this.element.appendChild(hpBar); } takeDamage(amount) { if (this.isDestroyed) return; this.hp = Math.max(0, this.hp - amount); this.hpBarInner.style.width = `${this.hp / this.maxHp * 100}%`; if (this.hp <= 0) this.destroy({ gainGauge: true }); } destroy(options = { gainGauge: false }) { if (this.isDestroyed) return; if (options.gainGauge) { Game.gauge = Math.min(100, Game.gauge + 25); Game.updateGaugeBar(); Game.addScore(this.scoreValue || 0); if (Math.random() < 0.3) { Game.add(new HealItem(this.x, this.y)); } } super.destroy(); } }
    ...
    class MovingPlatform extends GameObject { constructor(startX, startY, endX, endY, width, height, speed) { super('moving-platform'); this.type = 'moving-platform'; this.isPlatform = true; this.x = startX; this.y = startY; this.width = width; this.height = height; this.startX = startX; this.startY = startY; this.endX = endX; this.endY = endY; this.speed = speed; this.percent = 0; this.direction = 1; this.moveX = 0; this.moveY = 0; this.prevX = startX; this.prevY = startY; this.element.style.width = `${width}px`; this.element.style.height = `${height}px`; } update(deltaTime) { this.percent += (this.speed / 1000) * this.direction; if (this.percent >= 1) { this.percent = 1; this.direction = -1; } if (this.percent <= 0) { this.percent = 0; this.direction = 1; } this.x = this.startX + (this.endX - this.startX) * this.percent; this.y = this.startY + (this.endY - this.startY) * this.percent; this.moveX = this.x - this.prevX; this.moveY = this.y - this.prevY; this.prevX = this.x; this.prevY = this.y; } }
    class DisappearingPlatform extends GameObject { constructor(x, y, width, height) { super('disappearing-platform'); this.type = 'disappearing-platform'; this.isPlatform = true; this.x = x; this.y = y; this.width = width; this.height = height; this.state = 'visible'; this.timer = 0; this.disappearTime = 500; this.fadeTime = 300; this.reappearTime = 2000; this.element.style.width = `${width}px`; this.element.style.height = `${height}px`; } onPlayerStep() { if (this.state === 'visible') { this.state = 'fading'; this.timer = this.fadeTime; this.element.classList.add('fading'); setTimeout(() => { this.state = 'hidden'; this.timer = this.reappearTime; this.element.style.display = 'none'; this.element.classList.remove('fading'); }, this.disappearTime); } } update(deltaTime) { if (this.state === 'hidden') { this.timer -= deltaTime; if (this.timer <= 0) { this.state = 'visible'; this.element.style.display = 'block'; } } } }
