document.addEventListener('DOMContentLoaded', () => {
    ...
    const Game = {
        ...
        setupEventListeners() {
            document.addEventListener('keydown', e => this.keys[e.code] = true); document.addEventListener('keyup', e => this.keys[e.code] = false); document.getElementById('restart-button').addEventListener('click', () => this.startNewGame()); document.getElementById('restart-button-clear').addEventListener('click', () => this.startNewGame()); window.addEventListener('gamepadconnected', e => this.handleGamepadConnected(e)); window.addEventListener('gamepaddisconnected', e => this.handleGamepadDisconnected(e));
        },
        startNewGame() {
    ...
    class Player extends GameObject {
        ...
        draw(){
            ...
            // ▼▼▼ 変更: JSでtransformを完全に管理する ▼▼▼
            const scaleX = this.direction === "left" ? 1 : -1;
            let translateY = 0;

            // 走っている時だけ上下に動かす
            if (this.onGround && this.vx !== 0) {
                // アニメーション周期の前半は0px, 後半は-2px
